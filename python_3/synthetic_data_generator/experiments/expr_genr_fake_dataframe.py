...
def rand_shuf_loc(str_lst, rows_num):
    lst = adv_loc
...
def rand_shuf_prod(loc_list, rows_num):
    rand_shuf_str = [item for item in loc_list for i in range(rows_num)]
    random.shuffle(rand_shuf_str)
    return(rand_shuf_str)

# define function to generate random impression and click data
def rand_clic_impr(rows_num):
    rand_impr_lst = []
    click_lst = []
...
def rand_prod_price_discount(rows_num):
...
def rand_prod_click_timestamp(stime, etime, rows_num):
    prod_clik_tmstmp = []
    # start = "2020-10-01"
    # end = "2020-10-30"
    frmt = '%d-%m-%Y %H:%M:%S'
    ...
    for i in range(rows_num):
        ...
        time_stmp = [item for item in prod_clik_tmstmp for i in range(rows_num)]
...
def main():
    ...
    impression = rand_clic_impr(rows_num)
    clicks = rand_clic_impr(rows_num)
    product_price = rand_prod_price_discount(rows_num)
    product_discount = rand_prod_price_discount(rows_num)
    prod_clik_tmstmp = rand_prod_click_timestamp("20-01-2018 13:30:00",
                                                 "23-01-2018 04:50:34",rows_num)
    ...
    # fake_data = pd.DataFrame.from_dict(lst_dict)
